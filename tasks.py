...
class StockAnalysisTasks():
  def research(self, agent, company):
    return Task(description=dedent(f"""
      ...
      """),
      agent=agent,
      expected_output = """ Your final answer MUST be a report that includes a
        comprehensive summary of the latest news, any notable
        shifts in market sentiment, and potential impacts on 
        the stock.
        Also make sure to return the stock ticker. """
  ...
  def financial_analysis(self, agent): 
    return Task(description=dedent(f"""
      ...
      """),
      agent=agent,
      expected_output = """ Your final report MUST expand on the summary provided
        but now including a clear assessment of the stock's
        financial standing, its strengths and weaknesses, 
        and how it fares against its competitors in the current
        market scenario. """
  ...
  def filings_analysis(self, agent):
    return Task(description=dedent(f"""
      ...
      """),
      agent=agent,
      expected_output = """ Your final answer must be an expanded report that now
        also highlights significant findings from these filings,
        including any red flags or positive indicators for
        your customer."""
    )
  ...
  def recommend(self, agent):
    return Task(description=dedent(f"""
      ...
      """),
      agent=agent,
      expected_output = """ Your final answer MUST be a recommendation for your
        customer. It should be a full super detailed report, providing a 
        clear investment stance and strategy with supporting evidence.
        Make it pretty and well formatted for your customer. """
    )
